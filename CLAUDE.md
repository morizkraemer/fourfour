...
This project uses [**shed**](https://github.com/morizkraemer/shed) (a local dev shell CLI) for all dev commands. **Do not suggest raw `cargo` commands unless `shed` cannot do it.**

Available in this repo:
```bash
shed run waveform-dev <audio-file>    # Waveform dev harness (browser comparison vs Rekordbox)
shed run app                          # Start Tauri test UI dev server
shed status                           # Show service status
shed logs                             # Show recent logs
```

Scripts are defined in `shed.toml`. Add new ones with `shed script add <name> <cmd>`.
...
# Waveform dev harness (analyze + compare against Rekordbox side-by-side in browser)
shed run waveform-dev <audio-file>
```
